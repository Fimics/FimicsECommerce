...
//              return HttpStatus.INTERNAL_SERVER_ERROR;
    ...
    @PostMapping("/login")
    public FimJSONResult login(@RequestBody UserBO userBO,
                               HttpServletRequest request,
        ...
        if(userResult==null){
            return FimJSONResult.errorMsg("用户名或密码不能错误");
        }
        setNullProperty(userResult);
        CookieUtils.setCookie(request, response, "user",
                JsonUtils.objectToJson(userResult), true);
        return FimJSONResult.ok(userResult);
    }
    ...
    @PostMapping("/logout")
    public FimJSONResult logout(@RequestParam String userId,
                                  HttpServletRequest request,
                                  HttpServletResponse response) {
